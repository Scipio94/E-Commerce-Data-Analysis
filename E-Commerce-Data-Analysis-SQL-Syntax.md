...
SELECT
  ...
  ROUND(AVG(sub.MoM_Revenue_Diff_Pct) OVER(),2) AS MoM_Revenue_Diff_Pct_Avg
FROM
(SELECT 
  m.month,
  m.month_revenue,
...
SELECT 
   ...
   COALESCE(ROUND((Month_Customer_Count - Month_Customer_Count_Lag)/Month_Customer_Count_Lag,2),0) AS MoM_Customer_Diff_Pct
...
SELECT 
   ...
   COALESCE(ROUND((Month_Customer_Unique_Count - Month_Customer_Unique_Count_Lag)/Month_Customer_Unique_Count_Lag,2),0) AS MoM_Customer_Un_Diff_Pct
